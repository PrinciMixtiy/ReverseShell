...
@dataclass
class Server:
    """Server target class model.
    """
    addr: tuple = ()


class Client:
    """Client class that execute command on server.
    """
    ...
    def send_command(self, command: str, *args, show_progress=False) -> bytes:
        ...
        send_header_and_data(
            self.sock, final_command.encode(encoding=ENCODING))
        return recv_header_and_data(self.sock, show_progress=show_progress)

    def check_path_type(self, path: str) -> str:
        ...
        """
        return self.send_command('path-type', f'"{path}"').decode(encoding=ENCODING)
    ...
    def check_path_exists(self, path: str) -> bool:
        ...
        result = self.send_command(
            'path-exists', f'"{path}"').decode(encoding=ENCODING)
    ...
    def check_path_permission(self, path: str) -> bool:
        ...
        """
        result = self.send_command(
            'has-permission', f'"{path}"').decode(encoding=ENCODING)
        return json.loads(result)
    ...
    def screenshot(self, destination: str) -> None:
        ...
        """
        output = self.send_command('capture', show_progress=True)

        if output == 'screenshot-error'.encode(encoding=ENCODING):
            # Error for screenshot command
            print(colored_error("[Screenshot Error]") +
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                ...
                cwd = self.send_command('info').decode(encoding=ENCODING)
                prompt = ('╭─' + colored_info(f' {self.server.addr[0]}:{self.server.addr[1]}') +
                          f' {cwd}' + '\n╰─' + colored_info(' ❯ '))
                ...
                try:
                    ...
                    # For invalid command
                    print(colored_error(
                        '[Command Error] Your command is invalid.'))
                    ...
                    if splitted_command[0] == 'local':
                        ...
                        if command_splitter(cmd)[0] == 'cd':
                            if len(command_splitter(cmd)) == 2:
                                ...
                                print(colored_error(
                                    '[Argument Error]') + 'cd command need one parameter.')
                        else:
                        ...
                        if len(splitted_command) == 3:
                            self.download(
                                splitted_command[-2], splitted_command[-1])
                        elif len(splitted_command) == 2:
                            self.download(
                                splitted_command[-1], splitted_command[-1])
                        elif len(splitted_command) < 2:
                        ...
                        if len(splitted_command) == 2:
                            self.screenshot(splitted_command[1])
                        else:
                            print(colored_error('[Parameter Error]') +
                        ...
                        output = self.send_command(
                            command).decode(encoding=ENCODING)
                        print(output)

            except KeyboardInterrupt:
                continue

        print(colored_success(colored_success(
            colored_error('You are Disconnected.'))))
        self.sock.close()
